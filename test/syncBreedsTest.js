...
describe('#breedDetails', () => {
  ...
  it('returns "" if no description for breed', () => {
    const expectedDesc = "";
    const tabby = breedDetails('Tabby');
    assert.equal(expectedDesc, tabby);
  });
  it("returns undefined if that breed isn't in library", () => {
    const expectedDesc = undefined;
    const persian = breedDetails('Persian');
    assert.equal(expectedDesc, persian);
  });
